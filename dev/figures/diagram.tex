...
\usepackage[paperheight=6cm,paperwidth=11cm,margin=0cm]{geometry}
\usepackage{tikz}
\usetikzlibrary{calc}
...
v    \path [-latex] (exprs.south west) edge node[midway,left,color=green] {\texttt{defmap}} (sigts.north east);
...
%    \path[dashed] [-latex] (methods.south west) edge [bend left=100] node[pos=0.75,left] {file,line} (src.west);
    ...
    \path [-latex] (sigts.north west) edge [bend left=20] node[pos=0.4,left,color=green] {\texttt{sigtmap}} (exprs.west);
