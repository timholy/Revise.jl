...
- `exprstack`: used when descending into `@eval` statements:
  `ex` (used in creating the `ExInfo` object) is the first entry in the stack
- `allsigs`: a list of all method signatures defined by a given expression
- `structinfos`: a list of all type definitions defined by a given expression
