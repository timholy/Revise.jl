...
@testset "Revise" begin
    ...
    do_test("Comparison and line numbering") && @testset "Comparison and line numbering" begin
        ...
        @test length(dvs) == 3
        (def, val) = dvs[1]
        @test isequal(unwrap(def), RelocatableExpr(:(square(x) = x^2)))
        @test val == [Tuple{typeof(ReviseTest.square),Any}]
        @test firstline(unwrap(def)).line == 5
        m = @which ReviseTest.square(1)
        @test m.line == 5
        @test whereis(m) == (tmpfile, 5)
        @test RelocatableExpr(definition(m)) == unwrap(def)
        (def, val) = dvs[2]
        @test isequal(unwrap(def), RelocatableExpr(:(cube(x) = x^3)))
        ...
        @test RelocatableExpr(definition(m)) == unwrap(def)
        (def, val) = dvs[3]
        @test isequal(unwrap(def), RelocatableExpr(:(fourth(x) = x^4)))
        ...
        @test length(dvs) == 5
        (def, val) = dvs[1]
        @test isequal(unwrap(def),  RelocatableExpr(:(mult2(x) = 2*x)))
        @test val == [Tuple{typeof(ReviseTest.Internal.mult2),Any}]
        @test firstline(unwrap(def)).line == 13
        m = @which ReviseTest.Internal.mult2(1)
        @test m.line == 11
        @test whereis(m) == (tmpfile, 13)
        @test RelocatableExpr(definition(m)) == unwrap(def)
        (def, val) = dvs[2]
        @test isequal(unwrap(def), RelocatableExpr(:(mult3(x) = 3*x)))
