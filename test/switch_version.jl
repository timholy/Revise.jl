...
mktempdir() do thisdir
    ...
    @eval begin
        ...
        v2_cmd = """using Pkg; Pkg.activate("."); Pkg.develop(path = joinpath("$(dirname(@__FILE__))", "pkgs", "PkgChange_v2"))"""
        ...
        v1_cmd = """using Pkg; Pkg.activate("."); Pkg.develop(path = joinpath("$(dirname(@__FILE__))", "pkgs", "PkgChange_v1"))"""
